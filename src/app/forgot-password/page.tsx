...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = React.useCallback(
    async (values: ForgotValues): Promise<void> => {
      setLoading(true);
      try {
        const email = values.email.trim().toLowerCase();
        const existsResp = await fetcher("patient", `/patient/exists?email=${encodeURIComponent(email)}`);
        const exists = existsResp?.exists ?? existsResp?.data?.exists ?? false;
        ...
        const res: any = await creator("patient", "/patient/reset-password/request", { email });

       
        const msg =
          res?.message ||
          "If the account exists, we’ve sent password reset instructions to the email address.";
        openAlert("Check your email", msg, "success");
      } catch (e: any) {
        ...
        >
          ...
          >
            <Box display="flex" flexDirection={{ xs: "column", md: "row" }} flex="1">
              {/* Left illustration (same as login/reset) */}
              <Box
                display="flex"
                justifyContent="center"
                alignItems="center"
              ...
              >
                ...
                >
                  ...
                  <Typography
                    variant="body2"
                    textAlign="center"
                    sx={{ fontFamily: "Poppins", opacity: 0.85, mb: 1 }}
                  >
                    Enter the email you use for Arogyaa. We’ll send a link to reset your password.
                  </Typography>
                    ...
                    {({ values, errors, touched, handleChange, handleBlur, isSubmitting }) => (
                      <Form>
                        ...
                        <TextField
                          ...
                          sx={{
                            "& .MuiOutlinedInput-root": {
                              ...
                              "&.Mui-focused fieldset": { borderColor: "#7A4D9C" },
                              "& input": { paddingLeft: "3px", color: "black !important" },
                            },
                            "& .MuiInputLabel-root": { color: "#7A4D9C", fontFamily: "Poppins" },
                        ...
                        <Button
                          ...
                          startIcon={
                            isSubmitting || loading ? <CircularProgress size={22} /> : undefined
                          }
                        >
